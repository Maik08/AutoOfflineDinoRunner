...
def get_pixel_colors():
    ...
    bird_pixels = [latest_screenshot.getpixel(pos) for pos in bird_positions]
    
    return dino_pixel, cactus_pixels, bird_pixels  


threading.Thread(target=screenshot_thread, daemon=True).start()

while True:
    dino_pixel, cactus_pixels, bird_pixels = get_pixel_colors()
    
   
    if any(pixel == (172, 172, 172) for pixel in bird_pixels):  
        jumping()
